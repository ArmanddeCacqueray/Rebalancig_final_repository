...
#OR-Tools
#=================
def print_routes(routes, dropped=None):
    """
    Pretty print VRP routes.
    
    Parameters
    ----------
    routes : list[list[int]]
        Routes returned by the solver
    dropped : list[int]
        Dropped nodes
    """
    ...
    for v, route in enumerate(routes):
        ...
        print(f"Vehicle {v:02d} : {route_str}")

    if dropped is not None and len(dropped) > 0:
        print("\nDropped :", ", ".join(map(str, dropped)))

def plot_routes(routes, dropped, stations_df):
    """
    Spatial visualization of VRP routes.
    ...
    stations_df : pandas.DataFrame
        Must contain latitude, longitude, sign
    """
